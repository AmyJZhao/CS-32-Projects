...
int main()
{
    ...
    // destruction time.
    {
        ...
        for(int unsorted = 1; unsorted < m_sensors.size(); unsorted++)
        {
            Sensor* nextSensor = m_sensors[unsorted];
            int loc = unsorted;
            while(loc > 0 && compareSensorPtr(nextSensor, m_sensors[loc - 1]))
            {
                m_sensors[loc] = m_sensors[loc - 1];
                loc--;
            }
            m_sensors[loc] = nextSensor;
        }
